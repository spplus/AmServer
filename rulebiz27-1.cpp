...
RuleBiz27_1::RuleBiz27_1()
{
	flag = true;//判断已找到的开关与母刀连接点，是否有其他闭合的刀闸，之后的递归不再进行此逻辑
	flag2 = false;//已找到闭合刀闸，再次以此遍历时找到的元件集合，特殊处理的标志
}

bool RuleBiz27_1::topoByUnit(int saveid,string unitcim,STRMAP& passNodes,RMAP& ruleMap)
{
...
int RuleBiz27_1::topoBiz(int saveid,string unitcim,RMAP& ruleMap,string stationcim)
{
	...
	//27操作母刀才能进入27-1，重复的连接点已经单独处理，此时只可能找到线刀或母联刀闸的另一个母刀
